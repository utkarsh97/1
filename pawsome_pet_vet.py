...
def main():
    ...
    while answer in answers:
        ...
        if answer == 'i':
            ...
            result = vet.allowed_animal(kind)
            if result == 'True':
                urgency = -1
                while urgency not in range(0,101):
                    urgency = int(input("How urgently does the pet need help (0-100)?\n>"))
                    if urgency not in range(0,101):
                        print("Please enter (0-100)!")
                print(name, "is checked in!")
                animal.append(name_l)
                animal.append(kind)
                animal.append(urgency)
                animal_list.append(animal)
                print()
            else:
                print("Sorry... we cannot accept a", kind)
                print()
        elif answer == 'o':
            print(vet.print_animals(animal_list))
            pet = int(input("Please enter a number to check an animal out of the clinic\n>"))
            if pet not in range(1, len(animal_list) + 1):
                print("Error!")
                print()
            else:
                print(animal_list[pet - 1][0], "is checked out!")
                print()
                animal_list.pop(pet - 1)
        elif answer == 'a':
            sorted_list = vet.sort_by_name(animal_list)
            print(vet.print_animals(sorted_list))
        elif answer == 'm':
            animal = vet.get_most_urgent(animal_list)
            print(animal[0] + ", " + animal[1] + " with urgency " + str(animal[2]) + ", should be seen right away!")
            print()
        elif answer == 'l':
            animal = vet.get_least_urgent(animal_list)
            print(animal[0] + ", " + animal[1] + " with urgency " + str(animal[2]) + ", can wait!")
            print()
        elif answer == 'f':
