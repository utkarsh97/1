...
data = open("stats-clean.txt", "r")
data_ = data.read()
list_of_data = data_.split("\n")
new_list = []
final_list = []
player_list = []
...
for i in range(1, len(new_list) + 1):
    try:
        shooting_p = float(new_list[i][4])/float(new_list[i][5])
        if shooting_p >= 0.50 and int(new_list[i][3]) >= 50:
            player_list.append(new_list[i][0])
            player_list.append(format(shooting_p, '.2f'))
...
list_of_players = open("report.csv", 'w')
for i in range(len(final_list)):
    list_of_players.write(",".join(final_list[i]) + '\n')
list_of_players.close()
