...
type_of_player = input(name + " - are you a Warrior, Wizard or Programmer?\n>")
player = type_of_player + " " + name
length = int(len(player))
if length < 15:
    print(".xX WELCOME", player, "Xx.")
else:
    print("Welcome", type_of_player)
if type_of_player == 'Programmer':
    print("There is no treasure here for you. The real treasure is the time spent programming.")
elif type_of_player == 'Warrior' or type_of_player == 'Wizard':
    print("You're in a dimly lit room, standing in front of an enormous steel chest.")
    answer = input("Do you want to open the chest or leave the room?\n>")
    if answer == 'open' or answer == 'Open' or answer == 'OPEN': #I chose to use 'or' so its true if one of them is correct
        import random
        number = random.randint(1,20)
        if number <= 5:
            print("You are transported to Lulu Land! Where you will be eaten by monkeys!")
        elif number > 5 and number <= 10: #I chose to use and since both need to be fulfilled, also to set boundaries
            print("Nothing interesting happens!")
        elif number > 10 and number <= 15:
            print("You open the chest to find several stacks of floppy disks. Um. Yay?")
        else:
            print("You open the chest to find a stack of pizzas with ice cream on top? That's not gross, is it?")
    elif answer == 'leave' or answer == 'Leave' or answer == 'LEAVE':
        print("Nothing happens. Most. Boring. Game. Created.")
    else:
        print("You didn't open or leave the chest, and consequently a magic spell turns you into a tiny lemur. Things could we worse, I guess. Game over.")
else:
